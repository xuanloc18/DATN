...
public class CategoryController {
    ...
    public ResponseEntity<CategoryResponse> createCategory(@RequestBody CategoryCreateRequest categoryCreateRequest) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(this.categoryService.createCategory(categoryCreateRequest));
    }

    @PutMapping("/category/{id}")
    public ResponseEntity<CategoryResponse> updateCategory(
            @RequestBody CategoryCreateRequest category, @PathVariable int id) {
        return ResponseEntity.ok(this.categoryService.updateCategory(category, id));
    }

    @DeleteMapping("/category/{id}")
    public ResponseEntity<String> deleteCategory(@PathVariable int id) {
        if (this.categoryService.deleteCategory(id)) return ResponseEntity.ok("Xoa thanh cong");
        else return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("khong tim thay danh muc can xoa");
    }
