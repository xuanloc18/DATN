...
public class CartService implements ICartService {
    ...
    @Override
    @Transactional
    public void addProductToCart(int userId, int productId, String size, String color, int quantity) {
        ...
        if (this.userRepository.existsById(userId)) {
            ...
            //            ProductVariant productVariantOptional =
            // this.productVariantRepository.findProductVariantByProductAndColorAndSize(product,color,size);
            ProductVariant productVariantOptional =
                    this.productVariantRepository.findByProductColorAndSize(productId, color, size);
            System.out.println("Product ID: " + productId);
            System.out.println("Color: " + color);
            System.out.println("size: " + size);

            //            ProductVariant productVariantOptional =
            // this.testService.findProductVariant(productId,color,size);
            if (productVariantOptional != null) {
                ProductVariant realProduct = productVariantOptional;

                // kiem tra xem trong gio hang da tung co bien the san pham nay chua, neu co thi tang so luong, neu chua
                // thi them sp vao gio hang
                ...
                if (oldCartItem == null) // sp chua ton tai trong gio hang
                {
                    ...
                    double price = cart.getSumPrice()
                            + this.productRepository.findById(productId).get().getPrice() * quantity;
    ...
    public void deleteByVariantId(int userId, int variantId) {
        ...
        ProductVariant productVariant =
                this.productVariantRepository.findById(variantId).get();
        ...
        if (cartItemList.size() == 0) deleteCart(userId);
    }
