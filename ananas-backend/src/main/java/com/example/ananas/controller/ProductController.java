...
public class ProductController {
    ...
    public ResponseEntity<ProductResponse> updateProduct(
            @PathVariable int id, @RequestBody ProductCreateRequest productCreateRequest) throws IdException {
        if (!this.productService.exisById(id)) throw new IdException("id sản phẩm không tồn tại");
        return ResponseEntity.ok(productService.updateProduct(id, productCreateRequest));
    }

    // xoa san pham
    @DeleteMapping(
            "product/{id}") // luu y khi xoa san pham can xoa tat ca cac bien the cua no truoc moi có the xoa duoc
    public ResponseEntity<String> deleteProduct(@PathVariable int id) throws IdException {
        if (!this.productService.exisById(id)) throw new IdException("không tồn tại id sản phẩm");
    ...
    public ResponseEntity<String> addImages(@PathVariable int id, @RequestParam(name = "image") MultipartFile[] files)
    ...
    public ResponseEntity<String> deleteImages(@PathVariable int id) throws IdException {
        if (!this.productService.imagesExisById(id)) throw new IdException("id ảnh không tồn tại");
    ...
    public ResponseEntity<Integer> getSumOfProduct(
    ...
    public ResponseEntity<Integer> getSumOfProductVariant(@RequestParam(name = "variant_id") int variant_id)
            throws IdException {
